...
type CreateAccountHandler = ICommandHandler<
...
type PlaceOrderHandler = ICommandHandler<
  PlaceOrderCommand,
  { orderId: string; total: number }
>;
type GetOrderDetailsHandler = IQueryHandler<
...
describe("Type Tests: Command Handler Extraction", () => {
  test("ExtractCommandName extracts command name literal from handler", () => {
    ...
    type Test1 = Expect<
      Equal<ExtractCommandName<CreateAccountHandler>, "CreateAccount">
    >;
    type Test2 = Expect<
      Equal<ExtractCommandName<DepositMoneyHandler>, "DepositMoney">
    >;
    type Test3 = Expect<
      Equal<ExtractCommandName<PlaceOrderHandler>, "PlaceOrder">
    >;
  ...
  test("CommandPayload omits commandName from command interface", () => {
    ...
    type Test1 = Expect<
      Equal<
        CommandPayload<CreateAccountCommand>,
        { accountId: string; initialBalance: number }
      >
    >;
    type Test2 = Expect<
      Equal<
        CommandPayload<DepositMoneyCommand>,
        { accountId: string; amount: number }
      >
    >;
    type Test3 = Expect<
      Equal<
        CommandPayload<PlaceOrderCommand>,
  ...
  test("HandlerResult infers return type from handler execute method", () => {
    // Type-level assertions (compile-time)
    type Test1 = Expect<
      Equal<HandlerResult<CreateAccountHandler>, { id: string }>
    >;
    type Test2 = Expect<Equal<HandlerResult<DepositMoneyHandler>, void>>;
    type Test3 = Expect<
      Equal<
        HandlerResult<PlaceOrderHandler>,
        { orderId: string; total: number }
      >
    >;
...
describe("Type Tests: Query Handler Extraction", () => {
  test("ExtractQueryName extracts query name literal from handler", () => {
    // Type-level assertions (compile-time)
    type Test1 = Expect<
      Equal<ExtractQueryName<GetAccountBalanceHandler>, "GetAccountBalance">
    >;
    type Test2 = Expect<
      Equal<ExtractQueryName<GetOrderDetailsHandler>, "GetOrderDetails">
    >;
  ...
  test("QueryPayload omits queryName from query interface", () => {
    // Type-level assertions (compile-time)
    type Test1 = Expect<
      Equal<QueryPayload<GetAccountBalanceQuery>, { accountId: string }>
    >;
    type Test2 = Expect<
      Equal<QueryPayload<GetOrderDetailsQuery>, { orderId: string }>
    >;
  ...
  test("HandlerResult infers return type from query handler execute method", () => {
    ...
    type Test2 = Expect<
      Equal<
        HandlerResult<GetOrderDetailsHandler>,
        { orderId: string; status: string }
      >
    >;
...
describe("Type Tests: Multi-Module Union Aggregation (ModuleDefinition)", () => {
  ...
  test("CommandExecutors merges commands from multiple modules", () => {
    ...
    type HasCreateAccount = Expect<
      Equal<"CreateAccount" extends keyof Executors ? true : false, true>
    >;
    type HasDepositMoney = Expect<
      Equal<"DepositMoney" extends keyof Executors ? true : false, true>
    >;
    type HasPlaceOrder = Expect<
      Equal<"PlaceOrder" extends keyof Executors ? true : false, true>
    >;
  ...
  test("QueryExecutors merges queries from multiple modules", () => {
    ...
    type HasGetAccountBalance = Expect<
      Equal<"GetAccountBalance" extends keyof Executors ? true : false, true>
    >;
    type HasGetOrderDetails = Expect<
      Equal<"GetOrderDetails" extends keyof Executors ? true : false, true>
    >;
...
describe("Type Tests: Executor Function Type Inference (ModuleDefinition)", () => {
  ...
  test("command executor functions have correct input types", () => {
    ...
    type Test1 = Expect<
      Equal<CreateAccountInput, { accountId: string; initialBalance: number }>
    >;
    type Test2 = Expect<
      Equal<DepositMoneyInput, { accountId: string; amount: number }>
    >;
  ...
  test("query executor functions have correct output types", () => {
    // Type-level assertions for output types (unwrapped from Promise)
    type GetAccountBalanceOutput = Awaited<
      ReturnType<Queries["GetAccountBalance"]>
    >;
...
describe("Type Tests: ModuleDefinition Interface", () => {
  test("ModuleDefinition has correct shape with all properties", () => {
    ...
    type HasCommandHandlers = Expect<
      Equal<keyof AccountModule["commandHandlers"], "CreateAccount">
    >;
    type HasQueryHandlers = Expect<
      Equal<keyof AccountModule["queryHandlers"], "GetAccountBalance">
    >;
    type HasEventHandlers = Expect<
      Equal<keyof AccountModule["eventHandlers"], "AccountCreated">
    >;
...
describe("Type Tests: EventFlowsModule Interface", () => {
  test("EventFlowsModule has correct shape with name and setup", () => {
    ...
    type HasSetup = Expect<
      Equal<
        AccountEventFlowsModule["setup"] extends Function ? true : false,
        true
      >
    >;
...
describe("Type Tests: Edge Cases", () => {
  test("empty module has empty executor types (ModuleDefinition)", () => {
    ...
    type EmptyHandlers = {};
    type EmptyModule = ModuleDefinition<
      "empty",
      EmptyHandlers,
      EmptyHandlers,
      EmptyHandlers
  ...
  test("empty module factory has empty executor types", () => {
    ...
    // biome-ignore lint/complexity/noBannedTypes: Testing empty object type behavior
    type EmptyHandlers = {};
    type EmptyEventFlowsModule = EventFlowsModule<
      "empty",
      EmptyHandlers,
      EmptyHandlers,
      EmptyHandlers
