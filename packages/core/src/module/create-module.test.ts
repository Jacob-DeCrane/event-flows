...
class CreateAccountHandler
  implements ICommandHandler<CreateAccountCommand, { id: string }>
{
...
class DepositMoneyHandler
  implements ICommandHandler<DepositMoneyCommand, void>
{
...
class GetAccountBalanceHandler
  implements IQueryHandler<GetAccountBalanceQuery, number>
{
...
class GetAccountDetailsHandler
  implements
    IQueryHandler<GetAccountDetailsQuery, { id: string; balance: number }>
{
  async execute(
...
describe("createModule()", () => {
  test("creates module factory with command handlers", () => {
    ...
    expect(initialized.commandHandlers.CreateAccount).toBeInstanceOf(
      CreateAccountHandler,
    );
    expect(initialized.commandHandlers.DepositMoney).toBeInstanceOf(
      DepositMoneyHandler,
    );
  ...
  test("creates module factory with query handlers", () => {
    ...
    expect(initialized.name).toBe("accounts");
    expect(initialized.queryHandlers.GetAccountBalance).toBeInstanceOf(
      GetAccountBalanceHandler,
    );
    expect(initialized.queryHandlers.GetAccountDetails).toBeInstanceOf(
  ...
  test("creates module factory with event handlers", () => {
    ...
    const accountUpdatedHandler: EventHandler = async (
    ...
    expect(initialized.name).toBe("accounts");
    expect(initialized.eventHandlers.AccountCreated).toContain(
      accountCreatedHandler,
    );
    expect(initialized.eventHandlers.AccountUpdated).toContain(
  ...
  test("setup can use dependencies to create handlers with injected dependencies", () => {
    ...
    const handler = initialized.commandHandlers
      .CreateAccount as HandlerWithDependency;
