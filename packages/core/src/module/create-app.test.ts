...
class ListUsersHandler
  implements IQueryHandler<ListUsersQuery, Array<{ id: string }>>
{
...
class GetOrderHandler
  implements IQueryHandler<GetOrderQuery, { orderId: string; status: string }>
{
  async execute(
    query: GetOrderQuery,
...
type MockEventStore = EventStore & {
  _getPublisher: () => ((envelope: EventEnvelope) => void) | undefined;
};
...
describe("createEventFlowsApp()", () => {
  ...
  test("executes commands via namespaced API", async () => {
    ...
    const result = await app.commands.CreateUser({
      userId: "user-123",
      name: "John Doe",
    });
