...
class CreateOrderHandler
  implements ICommandHandler<CreateOrderCommand, { orderId: string }>
{
...
class GetOrderStatusHandler
  implements IQueryHandler<GetOrderStatusQuery, { status: string }>
{
...
describe("Module System Integration Tests", () => {
  ...
  test("query handler errors propagate to caller", async () => {
    ...
    await expect(
      app.queries.GetOrderStatus({ orderId: "order-fail" }),
    ).rejects.toThrow("Query handler failed intentionally");
  });
  ...
  test("multiple event handlers for same event are all invoked", async () => {
    ...
    const ordersModule = createModule({
      ...
      setup: ({ eventStore }) => ({
        ...
        eventHandlers: {
          OrderCreated: [() => handler1Called(), () => handler2Called()],
        },
      }),
    });
  ...
  test("event handler errors are swallowed and do not break other handlers", async () => {
    ...
    expect((errorReported.mock.calls[0][0] as Error).message).toBe(
  ...
  test("single handler module registers and executes correctly", async () => {
    ...
    // Query should work
    const result = await app.queries.GetOrderStatus({
